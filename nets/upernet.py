# -*- coding: utf-8 -*-
"""
Created on Sun Jul  9 15:51:13 2023
...
from .backbone.convnext_backbone import ConvNeXt_Backbone, convnext_dims

class AdaptiveMaxPoolWrapper(nn.Module):
    ...
    def forward(self, x):
        ...
        # 动态计算缩放因子
        scale_factor_h = torch.div(x.shape[2], self.output_size, rounding_mode='trunc')
        scale_factor_w = torch.div(x.shape[3], self.output_size, rounding_mode='trunc')
        # 使用 MaxPool 替代
        return nn.functional.max_pool2d(
            x, 
            kernel_size=(scale_factor_h, scale_factor_w),
            stride=(scale_factor_h, scale_factor_w)
        )
...
class PPM(nn.ModuleList):
    def __init__(self, pool_sizes, in_channels, out_channels):
        ...
        for pool_size in pool_sizes:
            self.append(
                nn.Sequential(
                    AdaptiveMaxPoolWrapper(pool_size), 
...
class MFF(nn.Module):
    ...
    def forward(self, Opt_x, Sar_x):
        out_puts = []
        for Opt_xi, Sar_xi in zip(Opt_x, Sar_x):
            out_puts.append(Opt_xi + Sar_xi)
        return out_puts

class UPerNet(nn.Module):
    def __init__(self, in_channels = 3, out_channels = 3, backbone_name = 'convnext_tiny', pretrained = True):
        super(UPerNet, self).__init__()
        self.out_channels = out_channels
        self.backbone = ConvNeXt_Backbone(backbone_name = backbone_name, pretrained = pretrained, in_channels = in_channels)
        
        self.in_channels = convnext_dims[backbone_name][-1]
        self.channels = 256
        # self.fusion = MFF()
        self.decoder = FPNHEAD(channels = self.in_channels)
        self.cls_seg = nn.Sequential(
            nn.Conv2d(self.channels, self.out_channels, kernel_size=3, padding=1),
            # nn.Sigmoid(),
        )
        self.name = "upernet_" + backbone_name
        
    def forward(self, inputs):
        x = self.backbone(inputs)
        # x_B = self.backbone_B(inputs_B)
        # x = self.fusion(x_A, x_B)
        x = self.decoder(x)
